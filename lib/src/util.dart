...
List<Object> calculateChatMessages(
  ...
  for (var i = messages.length - 1; i >= 0; i--) {
    ...
    final messageHascreated_time = message.created_time != null;
    final nextMessage = isLast ? null : messages[i - 1];
    final nextMessageHascreated_time = nextMessage?.created_time != null;
    final nextMessageSameAuthor = message.isAuthor == nextMessage?.isAuthor;
    ...
    // if (showUserNames) {
      ...
      // final isFirstInGroup = notMyMessage &&
          // ((message.author.id != previousMessage?.author.id) ||
              // (messageHascreated_time &&
                  // previousMessage?.created_time != null &&
                  // message.created_time! - previousMessage!.created_time! >
    ...
    // if (messageHascreated_time && nextMessageHascreated_time) {
      // nextMessageDateThreshold =
          // nextMessage!.created_time! - message.created_time! >= dateHeaderThreshold;
      ...
      // nextMessageDifferentDay = DateTime.fromMillisecondsSinceEpoch(
          ...
          // DateTime.fromMillisecondsSinceEpoch(
            // nextMessage.created_time!,
            // isUtc: dateIsUtc,
          // ).day;

      // nextMessageInGroup = nextMessageSameAuthor &&
          // message.id != lastReadMessageId &&
          // nextMessage.created_time! - message.created_time! <= groupMessagesThreshold;
    // }

    // if (isFirst && messageHascreated_time) {
      // chatMessages.insert(
        // 0,
        // DateHeader(
    ...
    if (message is types.ImageMessage) {
      if (kIsWeb) {
        if (message.content!.url!.startsWith('http') || message.content!.url!.startsWith('blob')) {
          gallery.add(PreviewImage(id: message.created_time!, uri: message.content?.url ?? ''));
        }
      } else {
        gallery.add(PreviewImage(id: message.created_time!, uri: message.content?.url ?? message.uri ?? ''));
      }
    }
  }
